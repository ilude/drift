...
const SYSTEM_CLASSES: SystemClass[] = [
	{ name: "peas-in-a-pod", weight: 0.4 },
	{ name: "solar-like", weight: 0.15 },
	{ name: "hot-jupiter", weight: 0.1 },
	{ name: "warm-jupiter-mixed", weight: 0.15 },
	{ name: "compact-multi", weight: 0.1 },
	{ name: "giant-dominated", weight: 0.1 },
];
...
function generateBinaryConfig(
	rng: () => number,
	primaryStar: StarResult,
	systemName: string,
): BinaryConfig {
	const roll = rng();
...
function generateWarmJupiterMixed(
	rng: () => number,
	planets: RawPlanetEntry[],
	starMass: number,
	binary: BinaryConfig,
...
function generateGiantDominated(
	rng: () => number,
	planets: RawPlanetEntry[],
	starMass: number,
	binary: BinaryConfig,
...
function generateComets(
	...
	for (let i = 0; i < count; i++) {
		...
		const period = Math.sqrt(a ** 3 / starMass);
