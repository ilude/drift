...
export const camera: THREE.PerspectiveCamera = new THREE.PerspectiveCamera(60, window.innerWidth / window.innerHeight, 0.1, 5000);
...
auMarkers.forEach((au: number) => {
    ...
    const geom: THREE.RingGeometry = new THREE.RingGeometry(r - 0.02, r + 0.02, 128);
    const mat: THREE.MeshBasicMaterial = new THREE.MeshBasicMaterial({ color: '#111118', side: THREE.DoubleSide });
