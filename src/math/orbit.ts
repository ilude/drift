import type { Vector3Like, PlanetCategory } from '../types';

export const DIST_SCALE: number = 100;
...
export function inclinedPosition(x: number, z: number, cosN: number, sinN: number, cosI: number, sinI: number): Vector3Like {
...
export function hillRadius(distAU: number, planetMassEarths: number, starMassSolar: number): number {
